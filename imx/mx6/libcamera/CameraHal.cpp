...
extern int camera_get_camera_info(int cameraId, struct camera_info* cameraInfo);
...
Pic_out:
    ...
    int CameraHal :: PrepareJpegEncoder()
    {
        ...
        //Compensate the default hardware layout rotation
        struct camera_info cameraInfo;
        int hwd_rotation;
        camera_get_camera_info(mCameraid,
                &cameraInfo);
        hwd_rotation = cameraInfo.orientation;
        rotate_angle += hwd_rotation;
        if(rotate_angle >= 360)
            rotate_angle -= 360;
